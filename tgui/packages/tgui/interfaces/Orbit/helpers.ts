...
export const getDepartmentByJob = (job: string): string | undefined => {
...
const getDepartmentColor = (job: string | undefined): string => {
  if (!job) return 'grey';
...
const getThreatColor = (orbiters = 0): string => {
  switch (true) {
...
export const sortByOrbiters = (poiA: Observable, poiB: Observable): number => {
